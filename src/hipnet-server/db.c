...
/*
 * HIPNET: Add routes for PD blocks handed out
 */
int
set_route(const struct ia_xx *ia) {
	int ret = 0;
	struct iasubopt *iasubopt;
	iasubopt = ia->iasubopt[0];
	static char ia_na[sizeof("ffff:ffff:ffff:ffff:ffff:ffff.255.255.255.255")];
	...
	case D6O_IA_PD:
		...
		sprintf(command, "route -A inet6 add %s/%d metric 1024 gw %s",
				addr_buf, (int)iasubopt->plen, ia_na);
		log_error("Command for PD route rules -> %s", command);
		int ret_val = system(command);
		if (ret_val != 0) {
			log_error("Could not assign downward IA_PD route rules");
			ret = -1;
		}
		break;
	default:
		log_error("Unknown ia type %u ",
			  (unsigned)ia->ia_type);
		ret = -1;
	}
...
write_ia(const struct ia_xx *ia) {
	...
	// HIPNET: Call our function to add PD routes
	set_route(ia);
