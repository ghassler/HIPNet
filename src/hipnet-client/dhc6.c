...
// HIPNET -> stop gap global variable
//extern char up_interface[30];
extern struct interface_info *up_interface;
struct client_state *current_best_client = NULL;
int current_best_client_plen = 0;
...
/* HIPNET -> our added function to compare lease options
 * and return a value indicating which is more "up"
 * This algorithm favors c0 as the existing best
 * as it was recieved first
 *
 * Return: 0 means c0 is up direction
 *         1 means c1 is up direction
 *         -1 error condition that should not be reached
 */
int
best_up_candidate(struct client_state *client)
{
	struct dhc6_ia *ia;
	struct dhc6_addr *addr;
	int client_plen = 128;

	log_debug("PRC: Comparing best up candidate");

	for (ia = client->selected_lease->bindings; ia != NULL; ia = ia->next) {
		if (ia->ia_type == D6O_IA_PD) {
			client_plen = (int)ia->addrs->plen;
		}
	}

	// Initial condition is where no client has ever been accepted as 
	// a favorable up detection
	// If we already have an up direction configured, the prefix length
	// will be set and this initial condition will not apply
	if (current_best_client == NULL && current_best_client_plen == 0) {
		current_best_client = client;
		current_best_client_plen = client_plen;
		return 1;
	}

	if (current_best_client_plen <= client_plen) {
		return 0; // favor existing best (arrived first)
	} else { 
		current_best_client = client;
		current_best_client_plen = client_plen;
		return 1;
...
/* HIPNET -> our added function to send the request on the
 * interface we have determined to be the "UP" direction.
 * all other interfaces are rejected by default
 */
void
send_request_up_direction(void *cpp)
{
	...
	for (ia = client->selected_lease->bindings; ia != NULL; ia = ia->next) {
		if (ia->ia_type == D6O_IA_PD) {
			log_debug("PRC: Up direction chosen is %s/%d",
				piaddr(ia->addrs->address), (int)ia->addrs->plen);
			log_debug("PRC:   X-- on interface %s",
				client->interface->name);
			up_interface = client->interface;
		}
	}

	current_best_client = NULL;
	do_select6(client);
}
...
start_selecting6(struct client_state *client)
{
	...
	// HIPNET
	// compare client with	current_best_client 
	if (best_up_candidate(client) == 1) {

		// Cancel old timer if it exists
		if (current_best_client != NULL) {
			cancel_timeout(send_request_up_direction, current_best_client);
		}

		// Set up new timer for best option
		/* Wait RT */
		tv.tv_sec = cur_tv.tv_sec + client->RT / 100;
		tv.tv_usec = cur_tv.tv_usec + (client->RT % 100) * 10000;
		if (tv.tv_usec >= 1000000) {
		...
		add_timeout(&tv, send_request_up_direction, current_best_client, NULL, NULL);
	}
