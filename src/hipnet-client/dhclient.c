...
/*
 * Copyright © 2013 Cable Television Laboratories, Inc. (CableLabs)
 ...
 *
 * Modifications were made by CableLabs to support HIPnet, learn more
 * about CableLabs at http://www.cablelabs.com
 */
...
struct interface_info *up_interface = NULL;
const char *path_dhclient_db = NULL;
//HIPNET - path to DHCPD6
const char *path_dhcpd6_conf = "/etc/dhcpd6.conf";
...
main(int argc, char **argv) {
	...
	if (wanted_ia_pd && (interfaces_requested != 1)) {
		//HIPNET ->  required for recursive self ordering Prefix delegation
		//usage();
	}
...
FILE *leaseFile = NULL;
//HIPNET delcare dhcpd6.conf
FILE *dhcpd6_conf_File = NULL;
		    ...
		    int rewrite, int sync)
{
	...
	// HIPNET -  dhcpd6.conf open
	dhcpd6_conf_File = fopen(path_dhcpd6_conf, "w");
		if (dhcpd6_conf_File == NULL) {
			log_error("can't create %s: %m", path_dhcpd6_conf);
			return ISC_R_IOERROR;
		}
	...
	//HIPNET -  print default options for dhcpd6.conf
	stat = fprintf(dhcpd6_conf_File, "authoritative;\n");
	if (stat <= 0)
		return ISC_R_IOERROR;

	stat = fprintf(dhcpd6_conf_File, "default-lease-time 3600;\n");
	if (stat <= 0)
		return ISC_R_IOERROR;

	stat = fprintf(dhcpd6_conf_File, "max-lease-time 7200;\n");
	if (stat <= 0)
		return ISC_R_IOERROR;

	//HIPNET - print dns server info here
	//TODO - currently setting DNS for DHCPD6 in dhclient-script
	//This should be completed here, only thing needed in DNS address
	
	//Example
	//stat = fprintf(dhcpd6_conf_File, "option dhcp6.name-servers 2001:470:20::2;\n");
	//if (stat <= 0)
	//	return ISC_R_IOERROR;
	...
	for (ia = lease->bindings ; ia != NULL ; ia = ia->next) {
		...
		for (addr = ia->addrs ; addr != NULL ; addr = addr->next) {
			if (ia->ia_type != D6O_IA_PD)
				...
				//HIPNET - print prefix6
				stat = fprintf(dhcpd6_conf_File,
					       "subnet6 %s/%d {\n",
					       piaddr(addr->address),
					       (int)addr->plen);
				if (stat <= 0)
            ...
            return ISC_R_IOERROR;
				...
				// HIPNET - only delegate more prefixes if I have more than /64
				if (addr->plen < 64) { 
					// HIPNET - Calculate the remainder of IPs set aside for
					// prefix delegation (keep first nibble sub-block
					// for myself)
					struct dhc6_addr pd_low_addr, pd_high_addr;
					int x;
					memcpy(&pd_low_addr, addr, sizeof(struct dhc6_addr));
					memcpy(&pd_high_addr, addr, sizeof(struct dhc6_addr));

					int loc = (int)pd_low_addr.plen / 8;  //location of subnet one nibble smaller
					pd_low_addr.address.iabuf[loc] += ((int)pd_low_addr.plen+4) % 8 == 0 ? 0x1 : 0x10;
					pd_high_addr.address.iabuf[loc] += ((int)pd_low_addr.plen+4) % 8 == 0 ? 0xf : 0xf0;

					stat = fprintf(dhcpd6_conf_File,
								"\tprefix6 %s",
								piaddr(pd_low_addr.address));
					...
					stat = fprintf(dhcpd6_conf_File,
								" %s /%d;\n",
								piaddr(pd_high_addr.address),
								(int)addr->plen+4);
				}

				//HIPNET - close subnet6 bracket and clean up
				stat = fprintf(dhcpd6_conf_File, "}\n");
				if (stat <= 0)
					return ISC_R_IOERROR;

				// HIPNET - Calculate ipv4 subnet mask based on the bits between
				// /48 and /52 on ipv6 prefix in the fashion
				// 10.X.X.0/24
				// TODO - Possible have server assign IPv4 addresses
				// based on IPv6 address
				unsigned char top_half = addr->address.iabuf[6];
				unsigned char bottom_half = addr->address.iabuf[7];
				log_error("IPV4 ADDRESS IS -> %x%x", top_half, bottom_half);
				log_error("IPV4 ADDRESS CONVERTS TO -> 10.%d.%d.0/24", top_half, bottom_half);

				// HIPNET - use this kind of statement to assign downward 
				// ip addresses (eth1, wlan1, etc...)
				// NOTE: this currently assigns one /64 for all downward
				// facing interfaces. (assumption that there will be
				// only one that can hand out PD blocks currently)
				struct interface_info *ip = interfaces;
				int count = 1;
				for (ip = interfaces; ip; ip = ip->next) {
					if (ip != up_interface) {
						// HIPNET - Workes based on the assumption that the low 
						// address in the range ip block (addr->address)
						// always ends in "::" allowing us to append our
						// specific address to the string
						char my_ip[80] = "";
						char my_command[200] = "";
						int radvd_inf = -1;
						int non_radvd_inf = -1;
						...
						// HIPNET - Interface [0] = lan (i.e. eth0.1)
						// Interface [1] = wan (i.e. eth0.2)
						if (!strcmp("eth0.2", ip->name)) { // Lan is up
							radvd_inf = 1;
							non_radvd_inf = 0;
						} else { // Wan is up
							radvd_inf = 0;
						...
						sprintf(my_command, "uci set radvd.@rdnss[%d].ignore=0", radvd_inf);
						// Run command to enable DNS in RAs on downward facing interface
						log_error("Command for setting DNS -> %s", my_command);
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot turn on DNS in radvd on interface %s", radvd_inf == 0 ? "lan" : "wan");
						}

						sprintf(my_command, "uci set radvd.@rdnss[%d].ignore=1", non_radvd_inf);
						// Run command to disable DNS in RAs on upward facing interface
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot turn off DNS in radvd on interface %s",
								non_radvd_inf == 0 ? "lan" : "wan");
						}

						sprintf(my_command, "uci set radvd.@interface[%d].ignore=0", radvd_inf);
						// Run command to enable RAs on downward facing interface
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot turn on radvd on interface %s", radvd_inf == 0 ? "lan" : "wan");
						}

						sprintf(my_command, "uci set radvd.@interface[%d].ignore=1", non_radvd_inf);
						// Run command to disable RAs on upward facing interface
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot turn off radvd on interface %s",
								non_radvd_inf == 0 ? "lan" : "wan");
						}

						// HIPNET - Keep the first /64 block for myself for hosts
						// connecting directly to this router
						// These addresses are delegated via radvd (SLACC)
						sprintf(my_command, "uci set radvd.@prefix[%d].ignore=0", radvd_inf);
						// Run command to enable address assignment
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot turn on radvd prefix on interface %s", radvd_inf == 0 ? "lan" : "wan");
						}
						sprintf(my_command, "uci set radvd.@prefix[%d].prefix=%s/64",
							radvd_inf, piaddr(addr->address));
						log_error("Command for setting prefix -> %s", my_command);
						// Run command to set prefix for address to be handed out
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot set radvd prefix on interface %s", radvd_inf == 0 ? "lan" : "wan");
						}

						//restart radvd
						system("/etc/init.d/radvd stop");
						system("/etc/init.d/radvd start");


						sprintf(my_ip, "%s%d", piaddr(addr->address), count);
						sprintf(my_command, "ip addr add %s dev %s", my_ip, ip->name);

						// Run command to assign ip address
						log_error("Command for down ip is -> %s", my_command);
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot assign down-facing ip address");
						}
						
						// HIPNET - Currently we only set ranges of 64 and so hardcode our routing as such
						sprintf(my_command, "route -A inet6 add %s/%d dev %s", piaddr(addr->address),
									64, ip->name);
						// HIPNET - Run command to assign routes for all range IA_NA addresses
						log_error("Command for down routing is -> %s", my_command);
						ret = system(my_command);
						if (ret < 0) {
							log_error("Cannot assign IA_NA routes");
						}
	...
	//HIPNET - do we need a flcose on dhcpd6_conf_File
	if (fflush(dhcpd6_conf_File) != 0)
		return ISC_R_IOERROR;
	//restart server
	system("/etc/init.d/dhcpd6 stop");
	system("/etc/init.d/dhcpd6 start");
